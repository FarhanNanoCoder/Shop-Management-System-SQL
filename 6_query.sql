...
----GETTING THE MANAGER OF SHOP ID=0-----
SELECT ID,F_NAME,E_ROLE 
FROM EMPLOYE
WHERE S_ID = 0
INTERSECT
...
---GETTING EMPLOYES OF SHOP WIHT ID=0---
SELECT ID,F_NAME,E_ROLE 
FROM EMPLOYE
WHERE S_ID = 0
MINUS
...
---GETTING EMPLOYEES AND MANAGER OF SHOP WIHT ID=0---
...
----CUSTOMER WITH MAX ID(SELF JOIN)-----

SELECT A.ID FROM CUSTOMER A MINUS
SELECT A.ID FROM CUSTOMER A JOIN CUSTOMER B ON A.ID<B.ID;
...
SELECT JOINED_AT FROM EMPLOYE;

SELECT ADD_MONTHS(JOINED_AT,2) AS TWO_MONTH_EXTENSION
FROM EMPLOYE;

SELECT LEAST (TO_DATE('1-JUN-2021'),TO_DATE('1-NOV-2021')) 
FROM DUAL;

SELECT GREATEST (TO_DATE('1-JUN-2021'),TO_DATE('1-NOV-2021'))
FROM DUAL;

SELECT LAST_DAY(JOINED_AT) FROM EMPLOYE;
...
INSERT INTO CUSTOMER(ID,F_NAME,L_NAME)
VALUES (8,'Farhan','Shariar');

SAVEPOINT C_1;

INSERT INTO CUSTOMER(ID,F_NAME,L_NAME)
VALUES (9,'Samiha','Afnan');
...
SET SERVEROUTPUT ON
DECLARE
   CURSOR EM_CURR IS SELECT ID,F_NAME,L_NAME FROM EMPLOYE;
   EM_REC EM_CURR%ROWTYPE;
begin
  OPEN EM_CURR;
  loop
    FETCH EM_CURR INTO EM_REC;
    EXIT WHEN EM_CURR%ROWCOUNT >3;
    DBMS_OUTPUT.PUT_LINE('ID: '||EM_REC.ID||' F_NAME: '||EM_REC.F_NAME||'L_NAME: '||EM_REC.L_NAME);
  end loop;
  CLOSE EM_CURR;
end;
/
